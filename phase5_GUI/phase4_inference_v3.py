"""
=============================================================================
PHASE 4 v3: Live Inference Pipeline (Tracker-Stabilised)
=============================================================================
Exam Proctoring — Pose-Based Suspicious Behaviour Detection

IMPROVEMENTS OVER v2:

  FIX 5 — Extended lost-track buffer (90 → 200 frames, ~7 seconds)
    v2's 3-second buffer caused frequent ID reassignments during brief
    occlusions (student leans forward, arm blocks neighbour). Extending
    to 7 seconds lets ByteTrack re-acquire the same person after longer
    occlusion gaps without creating a new track.

  FIX 6 — Minimum track length filter (MIN_TRACK_OBSERVATIONS = 30)
    Transient tracks (1-9 observations) were noise — momentary YOLO
    flickers that created PersonStates with no useful history. Tracks
    below 30 observations (~1 second) are now shown as "warming up"
    and excluded from classification, preventing them from inflating
    quiet-period false alarms.

  FIX 7 — Track graveyard + merge heuristic
    When a track is lost, its PersonState is saved in a graveyard with
    the last known bounding box. When a new track appears within
    MERGE_IOU_THRESHOLD of a recently deceased track (within
    GRAVEYARD_TTL_SEC seconds), the old PersonState — including its
    full classification history, feature buffer, and shoulder width
    stats — is transferred to the new track. This prevents the
    "history reset" problem that caused missed flags (e.g. V03's
    A1 passing_note at 00:55 was detected for 15 seconds but never
    flagged because a mid-event track break wiped the history).

  RETAINED FROM v2:
    - ByteTrack tracker (FIX 1)
    - Baseline calibration (FIX 2) — DISABLED by default (breaks
      feature distribution for models trained on raw features)
    - Stricter flagging rule 4-of-7 (FIX 3)
    - Looking-down suppression (FIX 4)

REQUIREMENTS:
    pip install mediapipe==0.10.14 opencv-python ultralytics numpy torch scikit-learn
    pip install supervision    # For ByteTrack (optional — falls back to IoU matching)

USAGE:
    python phase4_inference_v3.py --video test.mp4 --pkg inference_package_v11.pth
    python phase4_inference_v3.py --video_dir 03_long_test_videos/ --pkg inference_package_v11.pth --show
    python phase4_inference_v3.py --webcam --pkg inference_package_v11.pth
=============================================================================
"""
...
YOLO_CONF        = 0.5
PERSON_CLASS_ID  = 0
MAX_PEOPLE       = 10      # Raised from 8 to support demo videos with more people
...
# DISABLED by default — models trained on raw features break when baseline
# subtraction shifts the feature distribution to near-zero deviations.
# Enable only with models specifically trained on baseline-subtracted features.
BASELINE_ENABLED = False
BASELINE_SEC     = 10.0    # Seconds of calibration at start

# --- FIX 3: Stricter flagging rule ---
FLAG_WINDOW      = 7       # Look at the last 7 classification results (was 5)
FLAG_THRESHOLD   = 4       # Need 4 out of 7 to flag (was 3 out of 5)

# --- FIX 4: Looking-down suppression ---
LOOKDOWN_SUPPRESS        = True
LOOKDOWN_MAX_CONSECUTIVE = 3   # After 3 consecutive looking_down windows, suppress

# --- Tracker ---
# ByteTrack parameters (used when supervision is installed)
BYTETRACK_ACTIVATION_THRESHOLD = 0.4
BYTETRACK_LOST_BUFFER          = 200   # FIX 5: ~7 seconds at 30fps (was 90)
...
IOU_MAX_MISSING     = 200    # FIX 5: match ByteTrack buffer (was 90)

# --- FIX 6: Minimum track length ---
MIN_TRACK_OBSERVATIONS = 30  # ~1 second at 30fps — ignore transient flickers

# --- FIX 7: Track graveyard (for merge heuristic) ---
GRAVEYARD_TTL_SEC     = 15.0  # Keep dead tracks for up to 15 seconds
MERGE_IOU_THRESHOLD   = 0.3   # New track within this IoU of dead track → merge

# --- Visualisation ---
COLOR_NORMAL      = (0, 200, 0)      # Green
COLOR_SUSPICIOUS  = (0, 0, 220)      # Red
COLOR_WARNING     = (0, 200, 255)    # Yellow
COLOR_CALIBRATING = (200, 150, 0)    # Blue-ish — person is in baseline calibration
...
class PersonTracker:
    ...
    FIX 7: Maintains a 'graveyard' of recently lost tracks so that when a new
    track appears in a similar location, the old PersonState can be transferred
    instead of starting fresh.
    """

    def __init__(self, max_people=MAX_PEOPLE, fps=30.0):
        self.max_people = max_people
        self.fps = fps
        self.tracks = {}  # track_id → {'bbox': ..., 'missing': ...}

        # FIX 7: Graveyard for dead tracks
        # Maps old_track_id → {'bbox': last_bbox, 'death_frame': frame_idx}
        self.graveyard = {}
        self.frame_count = 0
    ...
    def _update_bytetrack(self, detections):
        ...
        # Move lost tracks to graveyard (FIX 7)
        ...
        ttl_frames = int(GRAVEYARD_TTL_SEC * self.fps)
        for gid in list(self.graveyard.keys()):
            if self.frame_count - self.graveyard[gid]['death_frame'] > ttl_frames:
        ...
        if track_ids and detections:
            ...
            for i, tid in enumerate(track_ids):
                ...
                for j, det in enumerate(detections):
                    db = det[:4]
                    costs[i, j] = 1.0 - self._compute_iou(tb, db)
    ...
    def find_graveyard_match(self, bbox):
        """
        FIX 7: Check if a bounding box matches a recently dead track.
        Returns the graveyard track_id if found, else None.
        """
        best_iou = MERGE_IOU_THRESHOLD
        best_gid = None
...
class InferenceEngine:
    ...
    def __init__(self, pkg_path):
        ...
        self.person_states = {}
        self.merge_count = 0  # FIX 7: track how many merges happen
        ...
        print(f'  Min track observations: {MIN_TRACK_OBSERVATIONS}')
        print(f'  Graveyard TTL: {GRAVEYARD_TTL_SEC}s, merge IoU: {MERGE_IOU_THRESHOLD}')
    ...
    def process_frame(self, frame, fps):
        ...
        for track_id, bbox in assignments:
            # FIX 7: If this is a new track, check graveyard for a merge
            if track_id not in self.person_states:
                graveyard_match = self.tracker.find_graveyard_match(bbox)
                if graveyard_match is not None and graveyard_match in self.person_states:
                    # Transfer the old PersonState to the new track ID
                    self.person_states[track_id] = self.person_states.pop(graveyard_match)
                    del self.tracker.graveyard[graveyard_match]
                    self.merge_count += 1
                else:
            ...
            # Classify (skip during calibration period and warming up)
            # FIX 6: Also skip if track has too few observations
            is_too_short = ps.total_frames < MIN_TRACK_OBSERVATIONS
            if ps.should_classify() and not ps.is_in_calibration and not is_too_short:
            ...
            is_warming = is_too_short and not is_new  # FIX 6: has enough frames for window but too few total
            results.append({
                'track_id': track_id,
                'bbox': bbox,
                'label': ps.current_label,
                'prob': ps.current_prob,
                'is_flagged': ps.is_flagged if not is_too_short else False,  # FIX 6
                'is_new': is_new,
        ...
        for tid in list(self.person_states.keys()):
            if tid not in active_ids and tid not in self.tracker.tracks:
                # Don't delete — graveyard might need it. Clean up only
                # if it's also gone from graveyard.
                if tid not in self.tracker.graveyard:
    ...
    def draw_annotations(self, frame, results, frame_idx, fps, total_frames=None):
        ...
        for r in results:
            ...
            elif r.get('is_warming', False):
                color = COLOR_UNTRACKED
                status_text = 'stabilising...'
            elif flagged:
...
def process_video(engine, video_path, output_dir, show=False):
    ...
    # Reset engine state
    engine.tracker = PersonTracker(fps=fps)
    engine.person_states = {}
    engine.merge_count = 0
    ...
    while True:
        ...
        if show:
            ...
            cv2.imshow(f'Phase 4 v3 — {stem}', preview)
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write(f'Phase 4 v3 Inference Summary\n{"="*30}\n\n')
        ...
        f.write(f'Lost track buffer: {BYTETRACK_LOST_BUFFER if HAS_SUPERVISION else IOU_MAX_MISSING} frames\n')
        f.write(f'Min track observations: {MIN_TRACK_OBSERVATIONS}\n')
        f.write(f'Track merges: {engine.merge_count}\n')

    print(f'\n  DONE: {video_path.name}')
    print(f'  Video: {out_video_path}')
    print(f'  Timeline: {timeline_path}')
    print(f'  Speed: {fps_proc:.1f} fps ({elapsed:.1f} sec)')
    print(f'  Track merges: {engine.merge_count}')
...
def main():
    parser = argparse.ArgumentParser(
        description='Phase 4 v3: Tracker-Stabilised Live Inference Pipeline',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python phase4_inference_v3.py --video exam.mp4 --pkg inference_package_v11.pth
    python phase4_inference_v3.py --video exam.mp4 --pkg inference_package_v11.pth --show
    python phase4_inference_v3.py --video_dir 03_long_test_videos/ --pkg inference_package_v11.pth
    python phase4_inference_v3.py --webcam --pkg inference_package_v11.pth
        """)
    ...
    parser.add_argument('--baseline', action='store_true', help='Enable baseline calibration (OFF by default)')
    ...
    if args.video:
        ...
        while True:
            ...
            cv2.imshow('Phase 4 v3 — Webcam', annotated)
